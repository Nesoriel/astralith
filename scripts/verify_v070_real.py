...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="astralith-v070-") as tmpdir:
        tmp_root = Path(tmpdir)
        db_path = tmp_root / "real-v070.db"
        ...
        try:
            ...
            status, actual = request_json(
                "POST",
                f"{base_url}/api/v1/gitops-repositories/actual-resources",
                token=token,
                payload={
                    "resource_type": "stack",
                    "resource_key": "uptime-kuma",
                    "source": "real-v070-verify",
                    "content": {"name": "uptime-kuma", "host": "monitoring-01", "image": "old/image:1"},
                },
            )
            assert status == 201 and isinstance(actual, dict), actual
            status, diffs = request_json(
                "POST",
                f"{base_url}/api/v1/gitops-repositories/{repository_id}/diff",
                token=token,
            )
            assert status == 200 and isinstance(diffs, list), diffs
            assert any(item["diff_type"] in {"create", "update"} for item in diffs), diffs
            status, plans = request_json(
                "GET",
                f"{base_url}/api/v1/gitops-repositories/{repository_id}/apply-plans",
            )
            assert status == 200 and isinstance(plans, list) and plans, plans
            status, policies = request_json(
                "GET",
                f"{base_url}/api/v1/gitops-repositories/{repository_id}/policy-results",
            )
            assert status == 200 and isinstance(policies, list) and policies, policies
            print(json.dumps({
                "status": "ok",
                ...
                "policy_statuses": sorted({item["passed"] for item in policies}),
