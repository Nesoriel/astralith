...
def seed_plan(db_path: Path, repository_id: int) -> int:
    ...
    try:
        conn.execute("UPDATE gitops_repositories SET last_commit_sha=? WHERE id=?", ("d" * 40, repository_id))
        cursor = conn.execute(
            """
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="astralith-v080-") as tmp:
        db_path = Path(tmp) / "real-v080.db"
        env = os.environ.copy()
        env["ASTRALITH_DATABASE_URL"] = f"sqlite:///{db_path}"
        proc = subprocess.Popen(
            ["uv", "run", "uvicorn", "backend.app.main:app", "--host", "127.0.0.1", "--port", "18080"],
            cwd=root,
            env=env,
        ...
        try:
            ...
            status, repo = req("POST", f"{base}/api/v1/gitops-repositories", token=token, payload={
                "name": "real v0.8.0 apply repo",
                "repo_url": "/tmp/infra",
                "branch": "main",
                "local_path": str(Path(tmp) / "checkout"),
                "auth_type": "none",
                "enabled": True,
            })
            assert status == 201, repo
            plan_id = seed_plan(db_path, repo["id"])
            status, approved = req("POST", f"{base}/api/v1/gitops-repositories/apply-plans/{plan_id}/approve", token=token)
            assert status == 200 and approved["status"] == "approved", approved
            status, run = req("POST", f"{base}/api/v1/gitops-repositories/apply-plans/{plan_id}/execute", token=token)
            assert status == 200 and run["stack_name"] == "uptime-kuma", run
            status, runs = req("GET", f"{base}/api/v1/gitops-repositories/{repo['id']}/apply-runs")
